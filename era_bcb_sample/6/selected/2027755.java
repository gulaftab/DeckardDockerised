...
public class AutoItAgent implements AutoIt {
    ...
    private static final String AUTOIT_REGISTRY_KEY = "\"HKEY_LOCAL_MACHINE\\SOFTWARE\\Microsoft\\Windows\\CurrentVersion\\App Paths\\AutoIt3{0}.exe\"";

    private static final Pattern PATTERN_EXTRACTING_AUTOIT_LOCATION = Pattern.compile("[A-E]\\:(?:\\\\[a-zA-Z\\s0-9\\(\\)]{2,20}){0,6}\\\\[a-zA-Z\\s0-9_]{2,20}\\.[a-zA-Z\\s0-9]{2,20}");
    ...
    public Map<String, Comparable<?>> runScript(String script, String workDir, String autoItLocation, int timeout) throws Exception {
        File sfile = File.createTempFile("autoit", ".au3");
        StringBuffer buf = new StringBuffer();
        buf.append("Local $var = ").append(script).append(NEW_LINE).append("Local $rc = @error\nConsoleWrite($var)\nExit($rc)\n");
    ...
    public boolean isProcessStillActive(String processName) throws Exception {
        ...
        return readFromStream("ErrorStream : ", child.getErrorStream()) && readFromStream("InputStream : ", child.getInputStream());
    }
